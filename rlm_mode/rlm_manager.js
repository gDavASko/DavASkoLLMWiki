...
const INDEX_PATH = path.join(SYSTEM_DIR, 'wiki-index.json');
...
export class RLMManager {
    ...
    _loadIndex() {
        ...
        return this.wikiIndex || { documents: {} };
    }
    ...
    list_layer(layerName) {
        ...
        const files = [];
        if (index.documents) {
            for (const [fileId, info] of Object.entries(index.documents)) {
    ...
    async spawn_worker(prompt, fileIds) {
        ...
        for (const fileId of fileIds) {
            const info = index.documents && index.documents[fileId];
            if (!info) {
                aggregatedContext += `[File ${fileId} not found]\n`;
                continue;
            }
            
            const absolutePath = path.join(SYSTEM_DIR, '..', info.path);
            try {
...
Когда соберешь всю нужную информацию, вызови инструмент finish.` 
        ...
        for (let i = 0; i < maxIterations; i++) {
            ...
            // Fallback parser for small models that output JSON in content instead of native tool_calls
            if ((!message.tool_calls || message.tool_calls.length === 0) && message.content && message.content.includes('"name"')) {
                try {
                    const match = message.content.match(/\{[\s\S]*"name"[\s\S]*\}/);
                    if (match) {
                        ...
                        if (parsed.name && parsed.arguments) {
                            ...
                            console.log(`[Manager] Recovered tool call from text content.`);
                        }
                    }
                } catch (e) {
                    // Ignore parsing errors, it was just a fallback
                }
            }
