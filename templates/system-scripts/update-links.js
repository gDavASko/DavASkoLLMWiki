...
function main() {
  ...
  const rawMappings = {
    // raw files
    'raw/project-docs/code_style.md': 'unity-wiki/raw/code_style.md',
    'raw/project-docs/code_style.md.meta': 'unity-wiki/raw/code_style.md.meta',
  ...
  filesToScan.forEach(f => {
    ...
    for (const [oldPath, newPath] of Object.entries(allMappings)) {
      ...
      // 2. Match relative path formats used inside wiki
    ...
    if (codeStylePattern.test(content)) {
      content = content.replace(codeStylePattern, 'unity-wiki/raw/code_style.md');
      changed = true;
    }
