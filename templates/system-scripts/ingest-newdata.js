...
function run() {
  ...
  layersInNewData.forEach(layer => {
    ...
    mdFiles.forEach(mdFilePath => {
      ...
      execSync(`node "${path.join(submoduleRoot, 'system', 'query-wiki.js')}" --ingest "${inputRelPath}" --layer ${layer} --subfolder "${subfolder}" --no-validate`, {
        stdio: 'inherit',
        cwd: submoduleRoot
      });
  ...
  try {
    execSync(`node "${path.join(submoduleRoot, 'system', 'lint-wiki.js')}"`, {
      stdio: 'inherit',
      cwd: submoduleRoot
    });
  } catch (err) {
