...
- **Change Logging**: Log detailed changes into the local log of the corresponding layer (UTF-8 with BOM): if the code changes in submodules (e.g., `Assets/KBPro/kbpro-modules`, etc.) — this is a core part, and log entries go to `kbpro-wiki/wiki/log.md`; if changes are in the main project (not in submodules) — this is a project-specific part, and log entries go to `dentistry-cow-wiki/wiki/log.md`. Then, append a fact reference in the root `log.md` with a link pointing directly to the new lines in the local log (e.g. `[layer/kbpro-wiki/wiki/log.md#L45-L52](../kbpro-wiki/wiki/log.md#L45-L52)`).
