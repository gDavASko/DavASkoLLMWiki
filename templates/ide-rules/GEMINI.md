﻿# Gemini CLI Rules for DavASko Project (Expert Persona)
...
## 1.1 DavASko Knowledge Base Flow
- AI knowledge base lives in `Assets/DavASko/davasko-ai-docs`.
- For architecture work, start from `Assets/DavASko/davasko-ai-docs/davasko-wiki/wiki/maps/architecture-map.md`.
- For gameplay, product, UI, audio, analytics, tutorial, and presentation work, start from `Assets/DavASko/davasko-ai-docs/davasko-wiki/wiki/maps/gameplay-product-map.md`.
- For AI rules, wiki maintenance, Bitrix import, and operational guardrails, start from `Assets/DavASko/davasko-ai-docs/llm-wiki/wiki/maps/operations-map.md`.
- For AI-generated code review and vibe-code cleanup, use `Assets/DavASko/davasko-ai-docs/unity-wiki/wiki/runbooks/ai-generated-code-review.md`, `Assets/DavASko/davasko-ai-docs/unity-wiki/wiki/runbooks/ai-code-cleaning.md`, and `Assets/DavASko/davasko-ai-docs/unity-wiki/wiki/concepts/unity-ai-code-review-checklist.md`.
- Use `Assets/DavASko/davasko-ai-docs/davasko-wiki/raw/` as source evidence after the relevant wiki map points you there.
...
- **Change Logging**: Log detailed changes into the local log of the corresponding layer (UTF-8 with BOM): if the code changes in submodules (e.g., `Assets/DavASko/davasko-modules`, etc.) — this is a core part, and log entries go to `davasko-wiki/wiki/log.md`; if changes are in the main project (not in submodules) — this is a project-specific part, and log entries go to `dentistry-cow-wiki/wiki/log.md`. Then, append a fact reference in the root `log.md` with a link pointing directly to the new lines in the local log (e.g. `[layer/davasko-wiki/wiki/log.md#L45-L52](../davasko-wiki/wiki/log.md#L45-L52)`).
- **Validation**: Run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Assets\DavASko\davasko-ai-docs\system/lint-wiki.ps1` after wiki or raw documentation changes.
...
- Follow `Assets/DavASko/davasko-ai-docs/davasko-wiki/raw/principals.md` (23 Architectural Principles).
- Follow `Assets/DavASko/davasko-ai-docs/davasko-wiki/raw/code_style.md` for naming and formatting.
