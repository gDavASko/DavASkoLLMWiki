...
- `Assets/KBPro/kbpro-ai-docs/unity-wiki/raw/code_style.md` - стиль C# и Unity-кода.
...
- **Логирование изменений**: Подробные изменения записывай в локальный `dentistry-cow-wiki/wiki/log.md` слоя (сохраняй в UTF-8 с BOM). В глобальный лог `Assets/KBPro/kbpro-ai-docs/log.md` добавляй только краткую запись о факте изменений со ссылкой на измененный локальный лог и диапазон строк (в формате `[layer/dentistry-cow-wiki/wiki/log.md#L45-L52](file:///path/to/log.md#L45-L52)`).
