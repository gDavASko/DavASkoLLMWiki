...
1. **Load the corpus.** Use `E:/UnityProjects/IRI/dentistry-cow/Assets/KBPro/kbpro-ai-docs/hyperresearch/.venv/Scripts/hyperresearch.exe note list --tag <corpus_tag> --all --json`
...
2. **Read breadth first.** For each note, read the title + summary + first
   ~400 chars (use `E:/UnityProjects/IRI/dentistry-cow/Assets/KBPro/kbpro-ai-docs/hyperresearch/.venv/Scripts/hyperresearch.exe note show <id> -j` and truncate). Do NOT read
