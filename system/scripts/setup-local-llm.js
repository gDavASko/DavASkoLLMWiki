...
function installOllamaWindows() {
    ...
    try {
        execSync(`powershell -Command "$ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri https://ollama.com/download/OllamaSetup.exe -OutFile '${installerPath}'"`, { stdio: 'inherit' });
...
export async function ensureLocalLLM(modelName) {
    ...
    console.log("💡 Совет: Если вы переместили Ollama на другой диск (например, на диск E:), убедитесь, что:");
    console.log("   1. Путь к новой папке Ollama добавлен в переменную среды PATH.");
    console.log("   2. Переменная среды OLLAMA_MODELS указывает на новую директорию с моделями.");
    console.log("   Иначе установщик может снова попытаться скачать её на диск C:.\n");
