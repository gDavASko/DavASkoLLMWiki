...
2. **Download Transcript (Bot Bypass)**: YouTube actively blocks standard scrapers. Prioritize using `yt-dlp` with the `--cookies-from-browser chrome` flag to bypass bot protections (see Extraction Guide).
...
7. **Actualize the auto-summaries (CRITICAL: PRESERVE ALL DETAILS)**:
   - Each generated `<layer>/wiki/sources/<name>.md` is a STUB that FAILS the linter. 
   - **DO NOT** just write a brief 3-sentence summary in the stub! The stub is the primary Wiki page the user reads.
   - You MUST copy the ENTIRE, fully-detailed markdown article you generated (with headers, mermaid diagrams, tables, and deep insights) directly into the stub. Keep the `YAML Frontmatter` intact, but replace the placeholder sections (`## Details`, etc.) with your massive structured article.
