...
## 3. Politika versionirovaniya i stale links (Политика версионирования и устаревших ссылок)

Для предотвращения устаревания информации и отслеживания целостности ссылок между слоями:
- **YAML Метаданные**: Каждая страница вики в заголовке frontmatter должна содержать поле `version: X.Y.Z`. При редактировании страницы её версия должна быть увеличена (minor/patch), а поле `last_updated: YYYY-MM-DD HH:MM` обновлено текущим локальным временем.
- **Ожидаемые версии в ссылках**: Все ссылки на другие страницы вики (`[[page-name]]`) должны явно указывать ожидаемую версию целевого документа, например: `[[page-name]] (vX.Y.Z)`.
- **Определение устаревания (Stale)**: Если версия целевого документа становится выше, чем указано в ссылающейся на него странице, ссылающаяся страница считается **устаревшей (stale)**.
- **Маркировка устаревших документов**: Устаревшие страницы должны переводиться в статус `status: stale`, содержать список устаревших ссылок в массиве `stale_links: []` в frontmatter и быть зарегистрированы в файле `wiki/stale-documents.md` соответствующего проекта.

---

## 4. Politika gaps (Политика пробелов полнотекстового поиска)
...
## 5. Структура каталогов и изоляция планов
...
│   ├── stale-documents.md      # Реестр устаревших документов, требующих обновления версий
